    ...
    <main class="card">
      <div class="field">
        ...
        <div class="box-p">
          ...
          <input id="peso" v-model.number="pesoKg" type="number" min="0" step="0.01" />
          <button @click="adicionarPeso" class="btn-sum"> <strong> + </strong> </button>
        </div>
      ...
      <div class="field">
        ...
        <input id="kgArroba" v-model.number="kgPorArroba" type="number" min="0.0" step="1" />
      ...
      <div class="field">
        ...
        <input id="qtd" v-model.number="preco" type="number" min="0" step="10" />
      </div>
      ...
      <div class="resultados">
        ...
        <div class="box-results">
          ...
          <div class="rslt-sect">
            ...
            <input id="inpult" v-model.number="nomeResultado" type="text" placeholder="ex.: pesagem-1" />
      ...
      <div class="resultados">
          ...
          <ul v-else class="history-list">
            <li v-for="(h, idx) in history" :key="h.id" class="history-item">
              <div class="entry" @click="selecionarHistorico(idx)" title="Clique para copiar o valor para o input">
                ...
                <div class="meta"><small>{{ h.label }} — total após: {{ Number(h.totalAfter).toLocaleString('pt-BR')
                    }}</small></div>
              </div>
        ...
        <div class="item-actions" v-if="history.length > 0">
          <button @click.stop="limparHistorico(idx)">✕</button>
        </div>
      </div>
    </main>
...
function adicionarHistoricoEntrada(value, totalAfter) {
  ...
  if (history.value.length > 1000) history.value.splice(1000)
}
...
function removeHistorico(index) {
...
.history-actions input[type="file"] {
  padding: 6px;
}
